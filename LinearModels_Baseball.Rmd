...
# add players' first and last name
players<- Master %>%
...
players %>% filter(year(debut)<1998) %>%
  ggplot(aes(salary,R_hat, color=POS)) + geom_point() +scale_x_log10()
```
...
# create a table with only the Rookie of the year (ROY) award winners and add their batting statistics
ROY <- AwardsPlayers %>%
...
mean(ROY$sophomore <= ROY$rookie)


```
```{r}
# do the similar analysis on all players that played the 2013 and 2014 seasons and batted more than 130 times
two_years <- Batting %>%
...
# what happens to the worst performers of 2013:
two_years %>% arrange('2013','2014')
...
fit <- falling_object %>% mutate(time_sq = time^2) %>%
